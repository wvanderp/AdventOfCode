...
function calcFuelForFuel(fuel) {
    const fuelRequired = calcFuelByMass(fuel);
    if (fuelRequired <= 0) {
        return 0
    } else {
        return fuelRequired + calcFuelForFuel(fuelRequired)
    }
...
function fuelForModule(module) {
    return module.map(calcFuelForFuel).reduce((a, b) => a + b, 0);
}
...
// console.log(fuelForModule([69]))
console.log(fuelForModule(masses))
